...
export function createSettingsForm(initialUserData, onCancelCallback, onSaveSuccessCallback) {
    ...
    container.innerHTML = `
        ...
        <div style="display: flex; flex-direction: column; gap: 10px; margin-bottom: 20px;">
            ...
            <div><label>个人介绍 (限100字)</label><textarea id="edit-intro" maxlength="100" rows="2" style="width:100%; padding:6px; background:#333; color:#fff; border:1px solid #555; border-radius:4px;">${userData.intro || ''}</textarea></div>
            <div style="display: flex; gap: 10px;">
                ...
                <div style="flex:1;"><label>国家</label><select id="edit-country" style="width:100%; padding:6px; background:#333; color:#fff; border:1px solid #555; border-radius:4px;">${countryOptions}</select></div>
                <div style="flex:1;"><label>地区</label><select id="edit-region" style="width:100%; padding:6px; background:#333; color:#fff; border:1px solid #555; border-radius:4px;"></select></div>
            </div>
        </div>
        ...
        <div style="display: flex; gap: 10px;">
            <button id="btn-save-settings" style="flex: 1; padding: 8px; background: #4CAF50; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: bold;">保存所有设置</button>
    ...
    // 级联地区选项逻辑
    const countrySelect = container.querySelector("#edit-country");
    const regionSelect = container.querySelector("#edit-region");
    const updateRegionOptions = () => {
        const regions = regionData[countrySelect.value] || [];
        regionSelect.innerHTML = regions.map(r => `<option value="${r}" ${r === userData.region ? 'selected' : ''}>${r}</option>`).join("");
    };
    ...
    container.querySelector("#btn-save-settings").onclick = async () => {
        ...
        try {
            ...
            savePrivacy("#hide-follows", "follows"); 
            savePrivacy("#hide-likes", "likes");
            savePrivacy("#hide-favorites", "favorites"); 
            savePrivacy("#hide-downloads", "downloads");
