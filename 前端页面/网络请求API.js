...
export const api = {
    ...
    async publishItem(itemData) { return request("/api/items", { method: "POST", body: itemData }); },
    async register(userData) { return request("/api/users/register", { method: "POST", body: userData }); },
    ...
    async followUser(userId, targetAccount, isActive) {
        return request("/api/users/follow", {
            method: "POST",
            body: { user_id: userId, target_account: targetAccount, is_active: isActive }
        });
    },
    
    async updatePrivacy(username, privacySettings) { return request(`/api/users/${username}/privacy`, { method: "PUT", body: privacySettings }); },
    async getItems(type = "tool", sort = "time", limit = 20) { return request(`/api/items?type=${type}&sort=${sort}&limit=${limit}`); },
    async getCreators(sort = "downloads", limit = 20) { return request(`/api/creators?sort=${sort}&limit=${limit}`); },
    
    async toggleInteraction(itemId, userId, actionType, isActive) {
        return request("/api/interactions/toggle", {
            method: "POST",
            body: { item_id: itemId, user_id: userId, action_type: actionType, is_active: isActive }
        });
    },
    async postComment(itemId, author, content, replyToUser = null, parentId = null) {
        return request("/api/comments", {
            method: "POST", body: { item_id: itemId, author, content, reply_to_user: replyToUser, parent_id: parentId }
        });
    },
