...
export function renderRegisterForm(container, switchView, onSuccessCallback) {
    ...
    avatarInput.onchange = (e) => {
        ...
        if (!file) return;
        if (file.size > 3 * 1024 * 1024) { avatarError.style.display = "block"; avatarInput.value = ""; selectedAvatarFile = null; avatarPreview.src = genderSelect.value === "male" ? DEFAULT_AVATAR_MALE : DEFAULT_AVATAR_FEMALE; return; }
        avatarError.style.display = "none"; selectedAvatarFile = file; 
        const reader = new FileReader(); reader.onload = (event) => avatarPreview.src = event.target.result; reader.readAsDataURL(file);
    };
    ...
    container.querySelector("#btn-submit-register").onclick = () => {
        ...
        if (!/^[a-zA-Z0-9_]{6,20}$/.test(account)) return alert("账号必须大于5个字符，且仅支持大小写英文字母、数字和下划线！");
        if (password !== pwdConfirm) return alert("两次输入的密码不一致！");
        if (!/^[A-Za-z0-9!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]{6,}$/.test(password)) return alert("密码必须大于等于6个字符，仅允许大小写字母、数字及常用标点！");
